...
fn is_detach_event(evtg: UINT, params: LPVOID) -> bool {
	let evtg = EVENT_GROUPS::from_bits_retain(evtg);
...
fn process_events(me: &mut dyn EventHandler, he: HELEMENT, evtg: UINT, params: LPVOID) -> BOOL
{
	...
	let result = if evtg == EVENT_GROUPS::SUBSCRIPTIONS_REQUEST {
		assert!(!params.is_null());
		...
		handled.is_some()
	} else if evtg == EVENT_GROUPS::HANDLE_INITIALIZATION {
		assert!(!params.is_null());
		...
		true
	} else if evtg == EVENT_GROUPS::HANDLE_SOM {
		assert!(!params.is_null());
		...
		false
	} else if evtg == EVENT_GROUPS::HANDLE_BEHAVIOR_EVENT {
		assert!(!params.is_null());
    ...
    use dom::event::EventReason;
		...
		handled
	} else if evtg == EVENT_GROUPS::HANDLE_SCRIPTING_METHOD_CALL {
		assert!(!params.is_null());
		...
		handled
	} else if evtg == EVENT_GROUPS::HANDLE_METHOD_CALL {
    assert!(!params.is_null());
